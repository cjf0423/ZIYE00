...
添加重写和mitm
使用方法，商店搜索快乐猜歌下载
先把重写禁用。进入游戏。随便猜一首歌获得数据。
脚本猜歌成功率百分之30左右。提现是顺序提现。
0.3-0.4-0.5-2-5-10-20
漏洞不凉。可一直提下去。。
[task_local]
#快乐猜歌无限刷
0 0 1 1 * klcg.js, tag=快乐猜歌无限刷, img-url=https://ae01.alicdn.com/kf/Uaade4566dd3744299956aa883f225386u.jpg, enabled=true
...
function ygkcck() {
   if ($request.url.indexOf("answer/reward?") > -1) {
...
let id = ygkcurl.replace(/id=\d+/,`id=${sjs}`)
//$.log(id)
